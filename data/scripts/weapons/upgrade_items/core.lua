...
-- onTargetCombat moved to data/scripts/eventcallbacks/player/onTargetCombat.lua
...
function us_onEquip(cid, iuid, slot)
    ...
    if player and item then
        ...
        local newBonuses = item:getAllEquippedBonuses()
        ...
        for i = 1, #newBonuses do
            ...
            if attr then
                if attr.combatType == US_TYPES.CONDITION then
                    ...
                    -- Use unique item UID as key instead of item ID to handle multiple items
                    if not US_CONDITIONS[bonusId][bonusValue][itemUid] then
                        US_CONDITIONS[bonusId][bonusValue][itemUid] = Condition(attr.condition)
                        if attr.condition ~= CONDITION_MANASHIELD then
                            US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(CONDITION_PARAM_SUBID,
                                1000 + player:getNextSubId(slot, i))
                            US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(attr.param,
                                attr.percentage == true and 100 + bonusValue or bonusValue)
                            US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(CONDITION_PARAM_TICKS, -1)
                        else
                            US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(CONDITION_PARAM_TICKS, 86400000)
                        end
                        US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(CONDITION_PARAM_BUFF_SPELL, true)
                        player:addCondition(US_CONDITIONS[bonusId][bonusValue][itemUid])
            ...
            if fromPosition.y >= 64 or fromPosition.x ~= CONTAINER_POSITION then
                ...
                if oldItem then
                    if oldItem:getType():isUpgradable() then
                        local oldBonuses = oldItem:getAllEquippedBonuses()
                        if oldBonuses then
                            local itemUid = oldItem:getUniqueId()
                            for key, value in pairs(oldBonuses) do
                                local attr = US_ENCHANTMENTS[value[1]]
                                if attr then
                                    if attr.combatType == US_TYPES.CONDITION then
                                        if US_CONDITIONS[value[1]] and US_CONDITIONS[value[1]][value[2]] and US_CONDITIONS[value[1]][value[2]][itemUid] then
                                            if US_CONDITIONS[value[1]][value[2]][itemUid]:getType() ~= CONDITION_MANASHIELD then
                                                player:removeCondition(
                                                    US_CONDITIONS[value[1]][value[2]][itemUid]:getType(),
                                                    CONDITIONID_COMBAT,
                                                    US_CONDITIONS[value[1]][value[2]][itemUid]:getSubId()
                                                )
                                            else
                                                player:removeCondition(
                                                    US_CONDITIONS[value[1]][value[2]][itemUid]:getType(),
                ...
                -- apply new item conditions (native + socketed skull bonuses)
...
ItemMovedEvent.onItemMoved = function(player, item, count, fromPosition, toPosition, fromCylinder, toCylinder)
    ...
    -- Remove all conditions (native + socketed skull bonuses)
    local bonuses = item:getAllEquippedBonuses()
    if bonuses then
        local itemUid = item:getUniqueId()
        for i = 1, #bonuses do
            ...
            if attr then
                if attr.combatType == US_TYPES.CONDITION then
                    if US_CONDITIONS[bonusId] and US_CONDITIONS[bonusId][bonusValue] and US_CONDITIONS[bonusId][bonusValue][itemUid] then
                        if US_CONDITIONS[bonusId][bonusValue][itemUid]:getType() ~= CONDITION_MANASHIELD then
                            player:removeCondition(
                                US_CONDITIONS[bonusId][bonusValue][itemUid]:getType(),
                                CONDITIONID_COMBAT,
                                US_CONDITIONS[bonusId][bonusValue][itemUid]:getSubId()
                            )
                        else
                            player:removeCondition(US_CONDITIONS[bonusId][bonusValue][itemUid]:getType(),
                                CONDITIONID_COMBAT)
                        end
                    end
                end
                ...
                for i = 1, #newBonuses do
                    ...
                    if attr then
                        if attr.combatType == US_TYPES.CONDITION then
                            ...
                            -- Use unique item UID as key instead of item ID
                            if not US_CONDITIONS[bonusId][bonusValue][itemUid] then
                                US_CONDITIONS[bonusId][bonusValue][itemUid] = Condition(attr.condition)
                                if attr.condition ~= CONDITION_MANASHIELD then
                                    US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(CONDITION_PARAM_SUBID,
                                        1000 + player:getNextSubId(slot, i))
                                    US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(
                                        attr.param,
                                        attr.percentage == true and 100 + bonusValue or bonusValue
                                    )
                                    US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(CONDITION_PARAM_TICKS, -1)
                                else
                                    US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(CONDITION_PARAM_TICKS,
                                        86400000)
                                end
                                US_CONDITIONS[bonusId][bonusValue][itemUid]:setParameter(CONDITION_PARAM_BUFF_SPELL, true)
                                player:addCondition(US_CONDITIONS[bonusId][bonusValue][itemUid])
...
GainExperienceEvent.onGainExperience = function(player, source, exp, rawExp)
    -- Ensure exp is a number, use rawExp as fallback
...
LookEvent.onLook = function(player, thing, position, distance, description)
    if thing:isItem() then
        if thing:getType():isUpgradable() then
            ...
            local itemAvailableSockets = thing:getMaxSockets()

            if upgrade > 0 or itemAvailableSockets then
                description = description:gsub(thing:getName(), "%1 +" .. upgrade .. " [" .. itemAvailableSockets .. "]")
            end
            ...
            if thing:isUnidentified() then
                ...
                for i = thing:getMaxSockets(), 1, -1 do
                    ...
                    local skullId = thing:getSocketedSkull(i)
                    if skullId then
                        local bonuses = thing:getSocketedSkullBonuses(i)
                        if bonuses then
                            for _, bonus in ipairs(bonuses) do
                                local attr = US_ENCHANTMENTS[bonus.enchantId]
                                if attr then
                                    description = description:gsub("Item Level: " .. itemLevel,
                                        "%1\n" .. attr.format(bonus.value))
                                end
                            end
                        end
                    else
                        -- Regular enchantment (non-skull socket)
...
function Item.rollAttribute(self, player, itemType, weaponType, unidentify)
    ...
    if unidentify then
        ...
        local slots = math.random(1, self:getMaxSockets())
        ...
        for i = 1, slots do
            ...
            if attrId == attackSpeedId then
                self:setAttribute(ITEM_ATTRIBUTE_ATTACK_SPEED, 0.5)
            end
        ...
        if bonuses then
            if #bonuses >= self:getMaxSockets() then
...
-- Returns ONLY native item bonuses (Slot{i}) - these are permanent and should never be removed by socket operations
...
-- Returns ALL bonuses for combat effects: native bonuses (Slot{i}) + socketed jewel skull bonuses
function Item.getAllEquippedBonuses(self)
    local data = {}

    -- Native bonuses (permanent)
    for i = 1, self:getMaxSockets() do
        local bonuses = self:getCustomAttribute("Slot" .. i)
        if bonuses then
            local t = {}
    ...
    -- Socketed jewel skull bonuses (removable)
    local socketedBonuses = self:getAllSocketedBonuses()
    if socketedBonuses then
        for _, bonus in ipairs(socketedBonuses) do
            data[#data + 1] = { bonus.enchantId, bonus.value }
        end
    end
    ...
    if itemType:getAttack() > 0 then
        ...
        if oldLevel < level then
            self:setAttribute(
                ...
                (self:getAttribute(ITEM_ATTRIBUTE_ATTACK) + finalValue) or
    ...
    if itemType:getDefense() > 0 then
        ...
        if oldLevel < level then
            self:setAttribute(
                ...
                (self:getAttribute(ITEM_ATTRIBUTE_DEFENSE) + finalValue) or
    ...
    if itemType:getHitChance() > 0 then
        ...
        if oldLevel < level then
            self:setAttribute(
                ...
                (self:getAttribute(ITEM_ATTRIBUTE_HITCHANCE) + finalValue) or
...
function Item.setUpgradeLevel(self, level)
    ...
    if itemType:getAttack() > 0 then
        if oldLevel < level then
            self:setAttribute(ITEM_ATTRIBUTE_ATTACK,
                self:getAttribute(ITEM_ATTRIBUTE_ATTACK) + (level - oldLevel) * US_CONFIG.ATTACK_PER_UPGRADE)
        else
            self:setAttribute(ITEM_ATTRIBUTE_ATTACK,
                self:getAttribute(ITEM_ATTRIBUTE_ATTACK) - (oldLevel - level) * US_CONFIG.ATTACK_PER_UPGRADE)
        end
    end
    if itemType:getDefense() > 0 then
        if oldLevel < level then
    ...
    if itemType:getArmor() > 0 then
        if oldLevel < level then
            self:setAttribute(ITEM_ATTRIBUTE_ARMOR,
                self:getAttribute(ITEM_ATTRIBUTE_ARMOR) + (level - oldLevel) * US_CONFIG.ARMOR_PER_UPGRADE)
        else
            self:setAttribute(ITEM_ATTRIBUTE_ARMOR,
                self:getAttribute(ITEM_ATTRIBUTE_ARMOR) - (oldLevel - level) * US_CONFIG.ARMOR_PER_UPGRADE)
        end
    end
    if itemType:getHitChance() > 0 then
        if oldLevel < level then
    ...
    --[[ self:setAttribute(ITEM_ATTRIBUTE_ATTACK_SPEED, 0.1) ]]
    if oldLevel < level then
...
function Item.identify(self, player, itemType, weaponType)
    ...
    if canUnique and math.random(US_CONFIG.UNIQUE_CHANCE) == 1 then
        ...
        local value = (math.random(0, US_MAX_SOCKET_PER_ITEM_TYPE[usItemType]))
        self:setMaxSockets(value)
    end
    return true
end

function Item.generateRandomSockets(self)
    local usItemType = self:getItemType()
    local maxSockets = US_MAX_SOCKET_PER_ITEM_TYPE[usItemType]

    if maxSockets and maxSockets > 0 then
        -- Allow 0 sockets again
        local value = math.random(0, maxSockets)
        self:setMaxSockets(value)
    else
        self:setMaxSockets(0)
    end
end
...
function Item.isUnique(self)
    return (self:getCustomAttribute("unique") or self:getCustomAttribute("item_unique_name")) and true or false
end
...
function Item.getItemUniqueName(self)
    return self:getCustomAttribute("item_unique_name")
end

function Item.hasItemUniqueName(self)
...
function Item.tryApplyUnique(self, corpsePosition)
    ...
    local itemName = self:getName():lower()
    local uniqueNames = ITEM_UNIQUE_MAP[itemName]

    if not uniqueNames or #uniqueNames == 0 then
        return false
    end
    ...
    if ITEM_UNIQUE_CONFIG.ANNOUNCE_UNIQUE then
        ...
        for i = 1, #specs do
            local player = specs[i]
            player:say(uniqueName .. "!", TALKTYPE_MONSTER_SAY, false, player, corpsePosition)
        end
    end
...
function Item.setMaxSockets(self, value)
    self:setCustomAttribute("max_sockets", value)
end

function Item.getMaxSockets(self)
    return self:getCustomAttribute("max_sockets") and self:getCustomAttribute("max_sockets") or 0
end
...
function MonsterType.calculateItemLevel(self)
    ...
    -- Add base randomness so even weak monsters can get a lucky higher roll
    local baseLevel = math.pow(monsterValue, 0.478)
    local variance = math.random(0, 5)
    local level = math.ceil(baseLevel + variance)
