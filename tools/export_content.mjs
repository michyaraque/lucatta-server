...
import { emitItemsXml, emitMonsterLua, getGeneratedMonsterFileName } from "./content-export/emitters.mjs";
...
const outputFiles = {
  ...
  monsterDirectory: path.join(tibiaServerRoot, "data", "scripts", "monsters"),
};
...
function cleanManagedLuaFiles(directoryPath) {
  ensureDirectory(directoryPath);
  for (const entry of fs.readdirSync(directoryPath, { withFileTypes: true })) {
    if (!entry.isFile() || !entry.name.endsWith(".lua")) {
      continue;
...
cleanManagedLuaFiles(path.join(tibiaServerRoot, "data", "scripts", "spells", "monster"));

cleanManagedLuaFiles(outputFiles.monsterDirectory);
